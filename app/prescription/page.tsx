...
export default function Home() {
  ...
  useEffect(() => {
    setLoggedIn(localStorage.getItem("nalamLoggedIn") === "true");
  }, []);
    ...
    <main className="min-h-screen bg-[#F7FAFA] text-[#0F172A]">
      <nav className="sticky top-0 z-50 border-b bg-white/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#0A2426] text-[#FF7A59] shadow-lg">
              <Stethoscope size={22} />
            </div>
            <div>
              <h1 className="text-xl font-bold text-[#0F8B8D]">Nalam</h1>
              <p className="text-xs text-slate-500">Wellness Within Reach</p>
            </div>
          </div>
          ...
          {loggedIn ? (
            <button
              onClick={logout}
              className="rounded-full bg-red-500 px-5 py-2.5 text-sm font-semibold text-white shadow-md"
            >
              Logout
            </button>
          ) : (
            <Link
              href="/login"
              className="rounded-full bg-[#0F8B8D] px-5 py-2.5 text-sm font-semibold text-white shadow-md"
            >
              Login
            </Link>
          )}
      ...
      <section className="relative overflow-hidden px-6 py-20">
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-12 md:grid-cols-2">
          <div>
            <div className="mb-6 inline-flex rounded-full bg-[#E3F1F1] px-4 py-2 text-sm font-semibold text-[#0F8B8D]">
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              ...
              <Link
                href="/doctors"
                className="rounded-full border border-[#0F8B8D] px-8 py-4 font-bold text-[#0F8B8D]"
              >
                Find Doctors
              </Link>
            </div>
          ...
          <div className="rounded-[2rem] bg-[#0A2426] p-6 text-white shadow-2xl">
            <div className="rounded-[1.5rem] bg-white/10 p-5">
              <div className="mb-6 flex items-center justify-between">
                <div>
                  <p className="text-sm text-white/60">AI Doctor Notes</p>
              ...
              <div className="my-5 flex justify-center">
                <div className="rounded-full bg-[#FF7A59]/20 px-4 py-2 text-sm text-[#FFB199]">
                  AI converting...
                </div>
              </div>
